{
  ...
  }
  {
    ...
    function getPerms(array: number[]): number[][] {
      ...
      const permsWithoutLast = getPerms(array);
      const permsWithLast: number[][] = [];
      for (let arr of permsWithoutLast) {
        for (let i = 0; i <= arr.length; i++) {
          permsWithLast.push([...arr.slice(0, i), last, ...arr.slice(i)]);
        }
      }
      return permsWithLast;
    }
  }
}
