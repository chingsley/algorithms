...

{
  ...
  {
    //   O(n * 4 * 4^n) time | O(n * 4^n) space
    // = O(4n * 4^n) time | O(n * 4^n) space
    // = O(n * 4^n) time | O(n * 4^n) space
    function phoneNumberMnemonics(phoneNumber: string): string[] {
      return backtrack(phoneNumber, 0);
    }
  ...
  {
    // O(n * 4^n) time | O(n * 4^n) space
    // Search "Reason:" to see explanation of he space compelixty defined above
    function phoneNumberMnemonics(phoneNumber: string): string[] {
      if (phoneNumber === "") return [""];

      const first = phoneNumber[0];
      const rest = phoneNumber.split('').slice(1).join("");
      const lettersOfFirst = letters[first];
      const wordsOfRest = phoneNumberMnemonics(rest);

      const result: string[] = [];
      for (let word of wordsOfRest) {
        for (let ch of lettersOfFirst) {
          result.push(ch + word);
        }
      }
